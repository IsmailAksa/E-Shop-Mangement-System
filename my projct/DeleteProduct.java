...
public class DeleteProduct extends JFrame implements ActionListener
{
  ...
  l2.setBounds(50,140,200,25);
		...
		t2.setBounds(250,140,200,25);t2.setToolTipText("Enter product name to delete product");
		jf.add(t2);
  ...
  l9.setBounds(470,180,200,25);
   ...
   l11.setBounds(470,260,180,25);
       ...
       l12.setBounds(470,300,180,25);
		...
		b1.setBounds(300,330,110,35);b1.setToolTipText("click to delete product details");
    ...
    if(ae.getSource()==b0)
	  {
	  	try
	  	{
	  	 if(((t2.getText()).equals(""))&&((t3.getText()).equals("")))
	        {
		    JOptionPane.showMessageDialog(this,"Please enter product company or name  !","Warning!!!",JOptionPane.WARNING_MESSAGE);
	        }
	        else
	        {
	         int foundrec = 0;
		...
		System.out.println("Connected to database.");
		  ...
		  ps=con.prepareStatement("select * from Product1 where Company='"+ t3.getText()+"' or ProductName='"+t2.getText()+"'");
		  rs=ps.executeQuery();
		  while(rs.next())
	      {
	 ...
	 {//delete
		...
		con=DriverManager.getConnection("jdbc:sqlite:D:/Product1.db");
		System.out.println("Connected to database.");
	    ps=con.prepareStatement("delete from Product1 where ProductName='"+t2.getText()+"' or Company='"+t3.getText()+"'");
	    ps.executeUpdate();
